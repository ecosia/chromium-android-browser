...
### Python 3 related issues and fixes

Please take a look at `.vpython3` to learn about the expected Python 3 version. Please make sure to use this specific version as there might be issues with the Python scripts when using diverging versions e.g.:

```
  File "src/third_party/catapult/third_party/gsutil/gslib/__main__.py", line 36, in <module>
    from six.moves import configparser
ModuleNotFoundError: No module named 'six.moves'
```

You might also want to consider using [pyenv](https://github.com/pyenv/pyenv) to manage your local Python installations.
...
```sh
export ANDROID_KEYSTORE_PASSWORD=[FROM BITWARDEN]
export ANDROID_APPCENTER_ID=[FROM BITWARDEN]
export CLOUDFLARE_CLIENT_ID=[FROM BITWARDEN]
export CLOUDFLARE_CLIENT_SECRET=[FROM BITWARDEN]
export FACEBOOK_APP_ID=[FROM BITWARDEN]
```
...
Instead of changing the color of views manually in java code or layout files, it is preferred to modify the [themes.xml](/components/browser_ui/styles/android/java/res/values/themes.xml) definition for `ThemeOverlay.BrowserUI.DynamicColors`, and add Ecosia material colors based on the [material styles guidelines](https://m3.material.io/styles/color/roles).
It might be possible that even the color changes in the themes file is not affecting the views as expected, especially if the Ecosia design diverges substantially from Chromium. In this case it would be preferred to adjust the colors in [ChromeColors.java](/components/browser_ui/styles/android/java/src/org/chromium/components/browser_ui/styles/ChromeColors.java).

After the introduction of the "surface_poslish" flag, the relationship of the theme colors seems to be as follows:
* colorSurface:
  * Toolbar 
  * Notification bar 
...
2. Compile, build, run the app and check if the selected color (green #00FF00) appears in any of the views.

3. If the color is present, then change the color for the corresponding semantic color that should be defined in the `ecosia_color_palette.xml` file
```gn
<item name="colorPrimary">@color/ecosia_brand_primary</item>
```

More information about Android styles and themes can be found [here](https://developer.android.com/develop/ui/views/theming/themes).
...
Or you can run the `scripts/prepare_build_folders.sh [version_name] [version_code]` script with new version numbers (this wont break anything but just update the build configurations).

Before building ensure that the `ecosia.keystore` is located in the `src` directory. For deployment follow [these steps](https://ecosia.atlassian.net/wiki/display/PROD/Release+Steps).
...
Are the ninjas having their way with you? They're a wiley bunch.. Check our [troubleshooting doc](https://ecosia.atlassian.net/wiki/spaces/DEV/pages/791707709/Troubleshooting+Android+Chromium) for some known issues and fixes.
...
* [AutofillEditorBase.java](chrome/android/java/src/org/chromium/chrome/browser/preferences/autofill/AutofillEditorBase.java)
* [BookmarkWidgetProvider.java](chrome/android/java/src/org/chromium/chrome/browser/bookmarkswidget/BookmarkWidgetProvider.java)
* [ChromeActivity.java](chrome/android/java/src/org/chromium/chrome/browser/ChromeActivity.java)
* [ClearBrowsingDataTabsFragment.java](chrome/android/java/src/org/chromium/chrome/browser/preferences/privacy/ClearBrowsingDataTabsFragment.java)
* [ConnectionInfoPopup.java](chrome/android/java/src/org/chromium/chrome/browser/page_info/ConnectionInfoPopup.java)
* [DataReductionPromoInfoBar.java](chrome/android/java/src/org/chromium/chrome/browser/infobar/DataReductionPromoInfoBar.java)
* [DataReductionProxySettings.java](chrome/android/java/src/org/chromium/chrome/browser/net/spdyproxy/DataReductionProxySettings.java)
* [EditorDialogToolbar.java](chrome/android/java/src/org/chromium/chrome/browser/widget/prefeditor/EditorDialogToolbar.java)
* [EphemeralTabSceneLayer.java](chrome/android/java/src/org/chromium/chrome/browser/compositor/scene_layer/EphemeralTabSceneLayer.java)
* [FaviconHelper.java](chrome/android/java/src/org/chromium/chrome/browser/favicon/FaviconHelper.java)
* [FeatureUtilities.java](chrome/android/java/src/org/chromium/chrome/browser/util/FeatureUtilities.java)
* [FirstRunFlowSequencer.java](chrome/android/java/src/org/chromium/chrome/browser/firstrun/FirstRunFlowSequencer.java)
* [FramebustBlockInfoBar.java](chrome/android/java/src/org/chromium/chrome/browser/infobar/FramebustBlockInfoBar.java)
* [HomeButton.java](chrome/android/java/src/org/chromium/chrome/browser/toolbar/HomeButton.java)
* [HomepageManager.java](chrome/android/java/src/org/chromium/chrome/browser/partnercustomizations/HomepageManager.java)
* [IncognitoNewTabPageView.java](chrome/android/java/src/org/chromium/chrome/browser/ntp/IncognitoNewTabPageView.java)
* [IncognitoNotificationManager.java](chrome/android/java/src/org/chromium/chrome/browser/incognito/IncognitoNotificationManager.java)
* [LaunchIntentDispatcher.java](chrome/android/java/src/org/chromium/chrome/browser/LaunchIntentDispatcher.java)
* [LocationBarLayout.java](chrome/android/java/src/org/chromium/chrome/browser/omnibox/LocationBarLayout.java)
* [LocationBarVoiceRecognitionHandler.java](chrome/android/java/src/org/chromium/chrome/browser/omnibox/LocationBarVoiceRecognitionHandler.java)
* [MainPreferences.java](chrome/android/java/src/org/chromium/chrome/browser/preferences/MainPreferences.java)
* [MenuDirectActionHandler.java](chrome/android/java/src/org/chromium/chrome/browser/directactions/MenuDirectActionHandler.java)
* [NativePageFactory.java](chrome/android/java/src/org/chromium/chrome/browser/native_page/NativePageFactory.java)
* [NearOomInfoBar.java](chrome/android/java/src/org/chromium/chrome/browser/infobar/NearOomInfoBar.java)
* [NearOomReductionInfoBar.java](chrome/android/java/src/org/chromium/chrome/browser/infobar/NearOomReductionInfoBar.java)
* [PageInfoView.java](chrome/android/java/src/org/chromium/chrome/browser/page_info/PageInfoView.java)
* [Preferences.java](chrome/android/java/src/org/chromium/chrome/browser/preferences/Preferences.java)
* [PrivacyPreferences.java](chrome/android/java/src/org/chromium/chrome/browser/preferences/privacy/PrivacyPreferences.java)
* [ReaderModeInfoBar.java](chrome/android/java/src/org/chromium/chrome/browser/infobar/ReaderModeInfoBar.java)
* [SavePasswordsPreferences.java](chrome/android/java/src/org/chromium/chrome/browser/preferences/password/SavePasswordsPreferences.java)
* [SearchActivity.java](chrome/android/java/src/org/chromium/chrome/browser/searchwidget/SearchActivity.java)
* [SearchWidgetProvider.java](chrome/android/java/src/org/chromium/chrome/browser/searchwidget/SearchWidgetProvider.java)
* [SendTabToSelfInfoBar.java](chrome/android/java/src/org/chromium/chrome/browser/send_tab_to_self/SendTabToSelfInfoBar.java)
* [SigninManager.java](chrome/android/java/src/org/chromium/chrome/browser/signin/SigninManager.java)
* [SingleCategoryPreferences.java](chrome/android/java/src/org/chromium/chrome/browser/preferences/website/SingleCategoryPreferences.java)
* [Tab.java](/home/jehmann/workspace/chromium/src/chrome/android/java/src/org/chromium/chrome/browser/tab/Tab.java)
* [TabListFaviconProvider.java](chrome/android/features/tab_ui/java/src/org/chromium/chrome/browser/tasks/tab_management/TabListFaviconProvider.java)
* [ToolbarLayout.java](chrome/android/java/src/org/chromium/chrome/browser/toolbar/top/ToolbarLayout.java)
* [ToolbarTablet.java](chrome/android/java/src/org/chromium/chrome/browser/toolbar/top/ToolbarTablet.java)
* [UpdateInfoBarController.java](chrome/android/java/src/org/chromium/chrome/browser/omaha/UpdateInfoBarController.java)
* [UpdateMenuItemHelper.java](chrome/android/java/src/org/chromium/chrome/browser/omaha/UpdateMenuItemHelper.java)
