...
AdblockTelemetryServiceFactoryBase::AdblockTelemetryServiceFactoryBase()
...
KeyedService* AdblockTelemetryServiceFactoryBase::BuildServiceInstanceFor(
  ...
  auto service = std::make_unique<AdblockTelemetryService>(
      GetSubscriptionService(context), url_loader_factory, GetInitialDelay(),
      GetCheckInterval());
  service->AddTopicProvider(std::make_unique<ActivepingTelemetryTopicProvider>(
      utils::GetAppInfo(), prefs, GetSubscriptionService(context),
...
bool AdblockTelemetryServiceFactoryBase::ServiceIsNULLWhileTesting() const {
  return true;
}

bool AdblockTelemetryServiceFactoryBase::ServiceIsCreatedWithBrowserContext()
    const {
  return true;
}

void AdblockTelemetryServiceFactoryBase::SetCheckAndDelayIntervalsForTesting(
