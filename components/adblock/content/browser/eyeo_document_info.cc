...
bool EyeoDocumentInfo::HasMatchedAcceptableAds() const {
  DCHECK(render_frame_host().IsInPrimaryMainFrame());
  return matched_acceptable_ads_;
}

void EyeoDocumentInfo::SetMatchedAcceptableAds() {
  DCHECK(render_frame_host().IsInPrimaryMainFrame());
  matched_acceptable_ads_ = true;
}

bool EyeoDocumentInfo::IsNavigationCounted() const {
  DCHECK(render_frame_host().IsInPrimaryMainFrame());
  return is_navigation_counted_;
}

void EyeoDocumentInfo::SetNavigationCounted() {
  DCHECK(render_frame_host().IsInPrimaryMainFrame());
  is_navigation_counted_ = true;
}
