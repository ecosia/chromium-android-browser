...
std::unique_ptr<AdblockResourceRequest> MakeSubscriptionRequest(
  ...
  return std::make_unique<AdblockResourceRequestImpl>(&kRetryBackoffPolicy,
                                                      url_loader_factory);
}
...
std::unique_ptr<TaskScheduler> MakeTaskScheduler() {
  return std::make_unique<TaskSchedulerImpl>(GetUpdateInitialDelay(),
...
MakeFilterConfigurationMaintainer(
  ...
  auto downloader = std::make_unique<SubscriptionDownloaderImpl>(
      base::BindRepeating(&MakeSubscriptionRequest, url_loader_factory),
