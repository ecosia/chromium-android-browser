...
class AdblockGeolocatedFilterListsTest
 ...
 public:
  AdblockGeolocatedFilterListsTest() : AdblockFilterListDownloadTestBase() {
    const auto testing_interval = base::Seconds(1);
    SubscriptionServiceFactory::SetUpdateCheckAndDelayIntervalsForTesting(
        testing_interval, testing_interval);
 ...
 protected:
  ...
  std::string recommendations_json_pattern_ = "[{\"url\": \"%s\"}]";
  std::string recommendations_json_pattern_2 =
      "[{\"url\": \"%s\"}, {\"url\": \"%s\"}]";
...
IN_PROC_BROWSER_TEST_F(AdblockGeolocatedFilterListsTest,
  ...
  // Let's push two lists in recommendations.json
  {
    ...
    auto waiter = GetSubscriptionInstalledWaiter();
    recommendations_json_ = base::StringPrintf(
        recommendations_json_pattern_2.c_str(), subscriptions[0].spec().c_str(),
  ...
  // clocks to make previous two auto installed lists expired
  {
    ...
    auto waiter = GetSubscriptionInstalledWaiter();
    recommendations_json_ = base::StringPrintf(
        recommendations_json_pattern_.c_str(), list_hu_.c_str());
    base::subtle::ScopedTimeClockOverrides time_override(
        []() {
...
IN_PROC_BROWSER_TEST_F(AdblockGeolocatedFilterListsTest,
  ...
  // Change recommendation to list_pl_ only
  {
    ...
    auto waiter = GetSubscriptionInstalledWaiter();
    recommendations_json_ = base::StringPrintf(
        recommendations_json_pattern_.c_str(), list_pl_.c_str());
    base::subtle::ScopedTimeClockOverrides time_override(
        []() {
