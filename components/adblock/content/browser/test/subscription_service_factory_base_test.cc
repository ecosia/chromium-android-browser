...
class SubscriptionServiceFactoryTestImpl
    : public SubscriptionServiceFactoryBase {
 public:
  ...
  SubscriptionService* GetSubscriptionService(
    ...
    return static_cast<SubscriptionService*>(BuildServiceInstanceFor(context));
  }

  void SetPrefs(TestingPrefServiceSimple* prefs) {
    prefs_ = prefs;
    adblock::common::prefs::RegisterProfilePrefs(prefs_->registry());
    adblock::PersistentFilteringConfiguration::RegisterProfilePrefs(
        prefs_->registry());
  }
 ...
 protected:
  ...
  const std::string& GetLocale() const override { return locale_; }

  std::string locale_ = "en-US";
  raw_ptr<TestingPrefServiceSimple> prefs_;
};

class SubscriptionServiceFactoryBaseTest : public testing::Test {
 public:
  SubscriptionServiceFactoryBaseTest() = default;
...
TEST_F(SubscriptionServiceFactoryBaseTest, TestFirstRun) {
  ASSERT_TRUE(
...
TEST_F(SubscriptionServiceFactoryBaseTest, TestFirstRunNonEnglishLocale) {
  auto polish_subscription = GURL{
  ...
  ASSERT_TRUE(std::find(subscriptions.begin(), subscriptions.end(),
                        DefaultSubscriptionUrl()) == subscriptions.end());
  ASSERT_TRUE(std::find(subscriptions.begin(), subscriptions.end(),
                        polish_subscription) != subscriptions.end());
  ASSERT_TRUE(std::find(subscriptions.begin(), subscriptions.end(),
                        AntiCVUrl()) != subscriptions.end());
}

TEST_F(SubscriptionServiceFactoryBaseTest, TestDisableAcceptableAdsSwitch) {
...
TEST_F(SubscriptionServiceFactoryBaseTest, TestDisableAdblockSwitch) {
  base::CommandLine::ForCurrentProcess()->AppendSwitch(
...
TEST_F(SubscriptionServiceFactoryBaseTest, TestDisableEyeoFilteringSwitch) {
  base::CommandLine::ForCurrentProcess()->AppendSwitch(
...
TEST_F(SubscriptionServiceFactoryBaseTest, TestDisableEyeoByDefault) {
...
TEST_F(SubscriptionServiceFactoryBaseTest, TestSettingsMigration) {
...
TEST_F(SubscriptionServiceFactoryBaseTest, TestSettingsMigrationWithFirstRun) {
  ASSERT_TRUE(
      prefs_.GetBoolean(common::prefs::kInstallFirstStartSubscriptions));
  // Now set legacy prefs.
  {
