...
"
for path in $CONTENT_SHELL_FILES
do
  git restore --source=$TOP_VANILLA_COMMIT --staged --worktree -- $path
done
git commit -n -m "Negative Content Shell module"
NEGATIVE_CONTENT_SHELL_MODULE_HASH=$(git rev-parse --short HEAD)
...
git revert -n $NEGATIVE_CONTENT_SHELL_MODULE_HASH
