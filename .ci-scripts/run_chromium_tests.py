...
def get_flake_list_name(test_binary_name):
    return test_binary_name + "_failed.txt"


def download_known_flakes_from_s3(test_binary_name, local_flake_list_path):
    remote_flake_list_path = f"{get_prefix()}/{get_flake_list_name(test_binary_name)}"
    ...
    try:
        ...
        s3_download_files.download_with_retry(
            bucket_name(), get_prefix(), get_flake_list_name(test_binary_name),
...
def upload_backup_of_flake_list(test_binary_name, local_flake_list_path):
    remote_flake_list_backup_path = f"{get_prefix()}/{get_flake_list_name(test_binary_name)}.bak"
    s3_upload_files.upload_files(
        local_flake_list_path, bucket_name(), get_prefix(),
        f"{get_flake_list_name(test_binary_name)}.bak")
...
def upload_flaky_tests_list(test_binary_name, local_flake_list_path):
    ...
    s3_upload_files.upload_files(local_flake_list_path, bucket_name(),
                                 get_prefix(),
                                 get_flake_list_name(test_binary_name))
...
def main(args):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        local_flake_list_path = os.path.join(
            temp_dir, get_flake_list_name(test_binary_name))
