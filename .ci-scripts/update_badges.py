...
def upload_files_to_minio(file_name):
    bucket = "wiki-resources"
    object_name = f"badges/{file_name}"
    ...
    try:
        ...
        # Retry once
        print(f"Retrying uploading {object_name} to minio")
        minio_client.fput_object(
        bucket_name=bucket,
        object_name=object_name,
        file_path=file_path,
...
def main(args):
    ...
    if args.update_release:
        ...
        upload_files_to_minio(image_file_name)
