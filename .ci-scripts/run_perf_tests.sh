...
runuser -p -u non_root -- time autoninja -j${NUMJOBS} -C out/Release components_perftests

# Run components_perftests for current branch
for i in $(seq 1 $SAMPLES_COUNT); do
  echo "Getting current sample $i"
  runuser -p -u non_root -- time xvfb-run -a ./out/Release/components_perftests --no-sandbox --gtest_filter="*Adblock*:*Eyeo*" > "components_perftests_current${i}.txt"
  runuser -p -u non_root -- cat "components_perftests_current${i}.txt" | sed -En "s/.*RESULT\ (.*)=\ ([0-9]*)\ (.*)/\1 (\3) \2/gp" > "components_perftests_current${i}_parsed.txt"
done
...
for i in $(seq 1 $SAMPLES_COUNT); do
  ...
  runuser -p -u non_root -- time xvfb-run -a ./out/Release/components_perftests --no-sandbox --gtest_filter="*Adblock*:*Eyeo*" > "components_perftests_baseline${i}.txt"
  runuser -p -u non_root -- cat "components_perftests_baseline${i}.txt" | sed -En "s/.*RESULT\ (.*)=\ ([0-9]*)\ (.*)/\1 (\3) \2/gp" > "components_perftests_baseline${i}_parsed.txt"
done
