...
#include "android_webview/browser/adblock/aw_content_security_policy_injector_factory.h"
...
bool IsFilteringNeeded(content::RenderFrameHost* frame) {
  if (frame) {
    auto* context = frame->GetProcess()->GetBrowserContext();
    if (context) {
 ...
 public:
  ...
  static void StartProxying(
    ...
    auto* self = static_cast<AdblockContextData*>(
        context->GetUserData(kAdblockContextUserDataKey));
    if (!self) {
      self = new AdblockContextData();
      context->SetUserData(kAdblockContextUserDataKey, base::WrapUnique(self));
    }
    auto* browser_context = android_webview::AwBrowserContext::FromWebContents(
...
AdblockContentBrowserClient::AdblockContentBrowserClient(
...
bool AdblockContentBrowserClient::CanCreateWindow(
  ...
  if (IsFilteringNeeded(opener)) {
    ...
    auto* subscription_service =
        adblock::SubscriptionServiceFactory::GetForBrowserContext(
            android_webview::AwBrowserContext::FromWebContents(web_contents));
...
bool AdblockContentBrowserClient::WillCreateURLLoaderFactory(
  ...
  auto* context = frame ? frame->GetProcess()->GetBrowserContext() : nullptr;
  ...
  if (use_adblock_proxy) {
    ...
    *factory_receiver = target_factory_remote.InitWithNewPipeAndPassReceiver();
    AdblockContextData::StartProxying(
        context, frame, render_process_id, std::move(proxied_receiver),
