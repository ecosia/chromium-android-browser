...
public class NotificationPermissionControllerTest {
    ...
    private NotificationPermissionController createNotificationPermissionController(
        ...
        return createNotificationPermissionController(rationaleDelegate,
    ...
    private void invokeOSPermissionCallback(
        ...
        int resultValue = permissionGranted ? PackageManager.PERMISSION_GRANTED
    ...
    public void testNotificationPrompt_alreadyHasPermission() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            // The permission request shouldn't show up on the third and fourth calls to
            // requestPermissionIfNeeded() as we already have permission.
            assertNotEquals(0, secondPermissionRequestTimestamp);
            assertEquals(secondPermissionRequestTimestamp,
                    PermissionPrefs.getAndroidNotificationPermissionRequestTimestamp());

            // We should record a histogram value indicating we've asked for permission 2 times,
            // showing the rationale and the OS prompt in one call to requestPermissionIfNeeded
            // counts as one attempt.
            verifyPermissionRequestCountHistogram(/* expectedCount= */ 2);
            // The last 2 calls to requestPermissionIfNeeded() should have recorded the state as
            // "Already has permission, rationale was shown before".
            verifyStateHistogramWasRecorded(NotificationPermissionState.ALLOWED,
                    /* expectedTimes= */ 2);
        });
    }
    ...
    public void testNotificationPromptShownOnStartup_noPermissionsYet_shouldShowOSPrompt() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            // The recorded state is "Never asked" because we record at the beginning.
    ...
    public void testNotificationPrompt_showOSPromptAndAccept() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            assertEquals(firstPermissionRequestTimestamp,
                    PermissionPrefs.getAndroidNotificationPermissionRequestTimestamp());

            // The first call to requestPermissionIfNeeded() should record a metric indicating we
            // have never asked before.
            verifyStateHistogramWasRecorded(
                    NotificationPermissionState.DENIED_NEVER_ASKED, /* expectedTimes= */ 1);
            // The second call to requestPermissionIfNeeded() should record a metric indicating the
            // permission was granted without having to show the rationale.
            verifyStateHistogramWasRecorded(NotificationPermissionState.ALLOWED,
                    /* expectedTimes= */ 1);
            // We should have recorded a metric indicating the OS prompt was accepted once.
    ...
    public void testNotificationPrompt_showOSPromptAndDismiss_tooSoonForSecondTime() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            assertEquals(firstOSPromptTimestamp,
                    PermissionPrefs.getAndroidNotificationPermissionRequestTimestamp());
            // The second call to requestPermissionIfNeeded() should have recorded the state as
            // "Asked once, dismissed, waiting to show again".
            verifyStateHistogramWasRecorded(NotificationPermissionState.DENIED_ASKED_ONCE,
                    /* expectedTimes= */ 1);
            // We should have recorded a metric indicating the OS prompt was rejected once.
    ...
    public void testNotificationPrompt_showOSPromptAndReject_tooSoonForSecondTime() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            // "Asked once, dismissed, waiting to show again".
            verifyStateHistogramWasRecorded(
                    NotificationPermissionState.DENIED_ASKED_ONCE, /* expectedTimes= */ 1);
            // We should have recorded a metric indicating the OS prompt was rejected once.
            verifyOSPermissionResultHistogram(false);
        });
    }
    ...
    public void testNotificationPrompt_showOSPromptAndDismiss_showAgainAfterTimePasses() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            // Dismiss OS prompt.
            invokeOSPermissionCallback(permissionDelegate, false);
            // If the permission dialog is dismissed then it doesn't count towards the request
            ...
            assertNotEquals(0, firstPermissionRequestTimestamp);
            assertNotEquals(0, secondPermissionRequestTimestamp);
            assertTrue(secondPermissionRequestTimestamp > firstPermissionRequestTimestamp);

            // We should record a histogram value indicating we've asked for permission 2 times.
            verifyPermissionRequestCountHistogram(/* expectedCount= */ 2);
            // The last call to requestPermissionIfNeeded() should have recorded the state as "Asked
    ...
    public void testNotificationPrompt_showOSPromptAndReject_showRationaleAfterTimePasses() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            long rationaleTimestamp = ContextUtils.getAppSharedPreferences().getLong(
    ...
    testNotificationPrompt_showOSPromptAndReject_showRationaleAndAccept_approveSecondOSPrompt() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            // As the rationale was accepted we should have also shown the permission prompt again.
            assertNotEquals(0, firstPermissionRequestTimestamp);
            assertNotEquals(0, secondPermissionRequestTimestamp);
            assertTrue(secondPermissionRequestTimestamp > firstPermissionRequestTimestamp);
            ...
            verifyPermissionRequestCountHistogram(/* expectedCount= */ 2);
            // The last call to requestPermissionIfNeeded() should have recorded the state as "Asked
            // once, denied, showing rationale", we record this state at the beginning of
            // requestPermissionNeeded, so it doesn't take into account the response to the shown
            // dialogs.
            verifyStateHistogramWasRecorded(NotificationPermissionState.DENIED_ASKED_ONCE,
                    /* expectedTimes= */ 1);
        });
    ...
    testNotificationPrompt_showOSPromptAndReject_showRationaleAndAccept_rejectSecondOSPrompt() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            // As the rationale was accepted we should have also shown the permission prompt twice.
            assertNotEquals(0, firstPermissionRequestTimestamp);
            assertNotEquals(0, secondPermissionRequestTimestamp);
            assertTrue(secondPermissionRequestTimestamp > firstPermissionRequestTimestamp);

            // If the user hits "Deny" again on the permission dialog then we can't request
            // permissions anymore, the third call to requestPermissionIfNeeded() should not show
            // anything.
            assertEquals(secondPermissionRequestTimestamp,
                    PermissionPrefs.getAndroidNotificationPermissionRequestTimestamp());
            ...
            // counts as one attempt.
            verifyPermissionRequestCountHistogram(/* expectedCount= */ 2);
            // The third call to requestPermissionIfNeeded() should have recorded the state as
            // "Asked twice, denied".
            verifyStateHistogramWasRecorded(
                    NotificationPermissionState.DENIED_ASKED_TWICE, /* expectedTimes= */ 1);
        });
    }
    ...
    public void testNotificationPrompt_showOSPromptAndReject_showRationaleAndReject() {
        mActivityScenarios.getScenario().onActivity(activity -> {
            ...
            assertEquals(secondPermissionRequestTimestamp,
                    PermissionPrefs.getAndroidNotificationPermissionRequestTimestamp());
            assertEquals(1, rationaleDelegate.getCallCount());
            // The last call should have recorded the state as "Asked once, showed rationale then
            // denied"
            verifyStateHistogramWasRecorded(NotificationPermissionState.DENIED_ASKED_TWICE,
                    /* expectedTimes= */ 1);
        });
    }
