...
public final class EcosiaFirstRunActivity extends AsyncInitializationActivity
    ...
    protected void triggerLayoutInflation() {
        ...
        firstRunContinueButton.setOnClickListener(view -> {
            ...
            mTrackingManager.sendIntroNextClickEvent(0);
            mTrackingManager.sendIntroDisplayEvent(1);
        });
    ...
    void next() {
        //As initial page Start 0 event is launched at start, next button is seen from Page 1,
        // add +1 to show the exact display pages
        mTrackingManager.sendIntroNextClickEvent(mPager.getCurrentItem() +1);
        nextPage();
    }
    ...
    void nextPage() {
        ...
        if (next < max) {
            ...
            final Singular singular = Singular.getInstance(ContextUtils.getApplicationContext());
            singular.trackEvent(Singular.Events.ONBOARDING_COMPLETE);
            finishOnboarding();
        }
    }
    ...
    public void onPageSelected(final int position) {
        if (position > 0) {
            mTrackingManager.sendIntroDisplayEvent(position+1);
        }
    }
    ...
    public static boolean launch(final Context context, final Intent fromIntent) {
        ...
        singular.trackEvent(Singular.Events.ONBOARDING_START);
